    ...
    return (
        <section className="relative w-full overflow-hidden" style={{ height: '100vh', minHeight: '600px' }}>
            ...
            >
                {heroSlides.map((s, i) => (
                    <div 
                        key={`bg-${s.src}-${i}`} // Unique key using index
            ...
            >
                {heroSlides.map((s, i) => (
                    <div 
                        key={`text-${s.src}-${i}`} // Unique key using index
                        className="relative flex-shrink-0 flex items-center px-6 h-full" 
                        style={{ width: `${100 / heroSlides.length}%` }}
                    >
            ...
            <button
                onClick={prev}
                className="absolute left-8 sm:left-18 top-1/2 -translate-y-1/2 z-30 w-12 h-12 rounded-full flex items-center justify-center border border-white/40 text-white/50 transition-all duration-300 hover:border-transparent hover:text-white"
            ...
            <button
                onClick={next}
                className="absolute right-8 sm:right-18 top-1/2 -translate-y-1/2 z-30 w-12 h-12 rounded-full flex items-center justify-center border border-white/40 text-white/50 transition-all duration-300 hover:border-transparent hover:text-white"
                style={{ backgroundColor: 'rgba(255, 94, 20, 0.4)' }}
                onMouseEnter={e => { (e.currentTarget as HTMLButtonElement).style.backgroundColor = PRIMARY; }}
                onMouseLeave={e => { (e.currentTarget as HTMLButtonElement).style.backgroundColor = 'rgba(255, 94, 20, 0.4)'; }}
                aria-label="Next"
